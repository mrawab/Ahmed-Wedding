...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // --- Countdown Timer ---
  const weddingDate = new Date("June 12, 2026 13:00:00").getTime();
      ...
      if (navigator.share) {
        try {
          await navigator.share({
            title: "دعوة زفاف أحمد وإيناس",
            text: "يسعدنا دعوتكم لحضور عقد قران وحفل زفاف أحمد وإيناس. انضموا إلينا للاحتفال!",
            url: currentUrl,
          });
        } catch (err) {
  ...
  // --- Envelope Opening Interactive Logic ---
  ...
  if (openBtn) {
    openBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      spawnGoldSparks(e, openBtn);

      // Start envelope opening sequence
      envelopeWrapper.classList.add("open");
      envelope.classList.add("open");

      // Slide up the letter tucked inside
      setTimeout(() => {
        if (letter) letter.classList.add("slide-up");
      }, 600);

      // Fade out envelope and reveal invitation page with confetti
      setTimeout(() => {
        ...
        if (typeof confetti === "function") {
          confetti({
            ...
            spread: 85,
            origin: { y: 0.6 },
            colors: ['#10365c', '#b59049', '#ecdba8', '#ffffff']
          });
        }
      }, 1500);
    });
